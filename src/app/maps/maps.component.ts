...
export class MapsComponent implements OnInit {
    ...
    polylines = [
        ...
        {lat: 6.8434248, lng: 79.9602304, speed: 40},
  ...
  ngOnInit() {
      this.polyline = this.rebuildPolylines();
  }
    ...
    private rebuildPolylines() {
        ...
        for (let i = 0; i < this.polylines.length; i++) {
            ...
            polyline[i].path = this.polylines[i];
        }
        if (polyline !== []) {
            if (polyline[polyline.length - 1].path.speed > 60) {
                this.truckIcon = './src/assets/img/red-truck-front.png';
            } else if (polyline[polyline.length - 1].path.speed > 5) {
                this.truckIcon = './src/assets/img/green-truck-front.png';
            } else {
                this.truckIcon = './src/assets/img/yellow-truck-front.png';
            }
            const endMarker = {
                lat: polyline[polyline.length - 1].path.lat,
                lng: polyline[polyline.length - 1].path.lng
            };
