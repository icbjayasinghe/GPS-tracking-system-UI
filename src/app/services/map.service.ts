...
export class MapService {
  ...
  getTrackingData(vehicle: any) {
      ...
      let url = 'http://123.231.52.227/api/vehicle/getTrackingData/' + JSON.parse(localStorage.getItem('user'))._id + '/' + vehicle;
      if (this.auth.findUser()) {
          url = 'http://123.231.52.227/api/vehicle/getTrackingData/' + vehicle;
      }
    ...
    getVehiclesCurrentLocation() {
        ...
        if (this.auth.findUser()) {
            url = 'http://123.231.52.227/api/vehicle';
        }
        return this.http.get(url, {headers: headers}).pipe(map(res => res.json()));
    }
  ...
  getVehicleHistory(historyObj: any) {
      ...
      headers.append('Authorization', this.auth.token);
      headers.append('Content-Type', 'application/json');
      const url = 'http://123.231.52.227/api/searchHistory';
